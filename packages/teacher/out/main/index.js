...
class DataManager {
  ...
  constructor(userDataPath) {
    ...
    this.testDataPath = path.join(this.dataDir, "test-sequences.json");
  }
  async init() {
    try {
  ...
  async saveTestData(data) {
    try {
      const cleanData = JSON.parse(JSON.stringify(data));
      await fs.promises.writeFile(this.testDataPath, JSON.stringify(cleanData, null, 2));
    } catch (error) {
  ...
  async loadTestData() {
    try {
      const data = await fs.promises.readFile(this.testDataPath, "utf-8");
...
const setupIpcHandlers = () => {
  electron.ipcMain.handle("play-audio", (event, path2) => audioManager?.playAudio(path2));
  electron.ipcMain.handle("pause-audio", () => audioManager?.pauseAudio());
  electron.ipcMain.handle("resume-audio", () => audioManager?.resumeAudio());
  electron.ipcMain.handle("stop-audio", () => audioManager?.stopAudio());
  ...
  electron.ipcMain.handle("save-test-data", async (event, data) => {
    try {
  ...
  electron.ipcMain.handle("load-test-data", async () => {
    try {
...
electron.app.whenReady().then(async () => {
  ...
  dataManager = new DataManager(electron.app.getPath("userData"));
