          ...
          <div class="space-y-6">
            ...
            >
                ...
                >
                  ...
                  <div class="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      ...
                      :class="option.value === getCorrectOption(index) ? 'bg-green-500' : 'bg-gray-400'"
                      :style="{
              ...
              <div class="mt-2 text-sm text-gray-600">
                平均解答時間: {{ questionStat.averageTime.toFixed(1) }}秒
              </div>
            </div>
          </div>
        </div>
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      ...props.stats.students.map(student => [
        ...
        `${student.correctRate.toFixed(1)}%`,
        `${(student.answers.reduce((sum, a) => sum + (a.timestamp - a.startTime), 0) / student.answers.length / 1000).toFixed(1)}秒`
      ]),
      // 空行
      [],
      // 問題別統計
